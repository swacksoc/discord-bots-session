...
You will need:
  ...
  - The Pycord library. Once you've installed python, follow [this page](https://docs.pycord.dev/en/master/installing.html) to get it installed. Should be as easy as entering `python -3 -m pip install -U py-cord` in the command prompt.
...
splitMsg = message.content.split(" ")
    await message.reply(" ".join(splitMsg))
```
...
```Python
splitMsg = message.content.split(" ")
    await message.reply(" ".join(splitMsg[1:]))
```
